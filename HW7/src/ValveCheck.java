...
public class ValveCheck extends Atomic {//////////////output could be int or rocket
    ...
    public String deltaConfluent(ArrayList b, int index, double e) {
        if (internalStates.size() < 1) {
            ...
            if (rocket.getInternalPressure() <= (5+(random.nextInt(2))-(random.nextInt(2)))) {//pass (0, rocket)(rocket.getInternalPressure() < (5+(random.nextInt(3))-(random.nextInt(3))))
                outputs.set(0, 0);
